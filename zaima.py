...
class zaima:
    ...
    def __bo_time(self):
        self.__boData = {}
        self.__boData['nonB'] = datetime.now() - \
            datetime.strptime(self.__roomData.get(
    ...
    def __read_langs(self):
        ...
        if path.exists(self.langsFile) and path.exists(self.nameFile):
            ...
            self.__quinLanguage = random.choice(
                open("/Users/yven/Dropbox/Workspace/Github/zaima/langs/quinLangs.dat", "r").readlines()).strip()
            self.__quinName = random.choice(
                open("/Users/yven/Dropbox/Workspace/Github/zaima/langs/quinName.dat", "r").readlines()).strip()
